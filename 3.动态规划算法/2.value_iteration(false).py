...
def value_update():  
    '''
    1st difference from policy update: change the function name "policy_evaluation -> value_update"
    '''
    ...
    for row in range(4):
        for col in range(12):
            ...
            #  new_values[row, col] = action_value.sum() # the state value
                                                      # is calculated by
                                                      # adding every action value * its policy (probability) 
            '''求每一个格子的value，等于该格子下所有动作的最大action value, 也就是，无需做小迭代，直接计算一个贪心的vk（因此这时就不是Vk*，policy_iteration就是，因为有小迭代）'''
            new_values[row, col] = action_value.max()


    return new_values
'''真的是这样的嘛？存疑？其实还是要计算的我注释掉的那几步，但是在迭代的过程中，仅进行1次值迭代就可以了（也就是应该在后面train的地方修改）'''
...
# train
for _ in range(3):  # 3次刚好是临界点，你可以多训练几次，就收敛了
